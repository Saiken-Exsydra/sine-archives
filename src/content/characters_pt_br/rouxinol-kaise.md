...
> — Anotação em um relatório de campo Pager, distrito de Vrenne, origem da fonte censurada
...
Ela carrega a insígnia de Stylus modificada com a marca de certificação dupla. Não existia um modelo oficial para essa modificação. O Registry Branch precisou desenvolver um especificamente para este arquivo.
...
Segundo o Index Theorem, a Redactory classe Mirror é entendida como uma Redactor cuja Anchor se estabiliza através de alinhamento conceitual com correspondência — a propriedade relacional entre uma estrutura e uma réplica precisa dessa estrutura. Na prática, isso permite acessar, através do Dive, roteamentos de configuração que se alinham de maneira extremamente próxima da assinatura estrutural de outro Redactor ou indivíduo portador de Resonance.

**Correspondência de Resonance** — Durante um Dive sustentado em proximidade direta com outro Redactor ou indivíduo portador de Resonance, Rouxinol consegue direcionar sua seleção de configurações para uma expressão paralela à Anchor estrutural do alvo. O efeito não é identidade — ela não se torna o outro Redactor, não acessa o caminho de Dive da outra pessoa e não replica resultados fora das configurações disponíveis para ela mesma. Em Depth de Stylus e em curta distância, consegue sustentar essa correspondência por vários minutos sem risco de Overreach.

**Leitura de Superfície** — Em Dive superficial, sem ativar correspondência completa, ela consegue identificar a Anchor de um Redactor através de uma breve proximidade — em alguns casos registrados, após observar apenas um único evento de manifestação. Ela não consegue ler Anchors que nunca tenham se manifestado em sua presença.

**Memória de Correspondência** — Quando a lâmina-artefato é usada para eliminar um alvo, a assinatura de Resonance da Anchor desse alvo é retida no loop secundário da Threshold Anchor como uma impressão estrutural recuperável. A Mirror Anchor então consegue direcionar a correspondência para essa impressão preservada sem precisar da proximidade de uma fonte viva.
...
**Retenção de Impressão** — O alinhamento da Threshold Anchor com condições de fronteira permite que ela retenha, como configurações indexadas acessíveis, impressões de Resonance das Anchors de indivíduos processados pela lâmina. A observação oficial do painel de avaliação registra: *“Conseguimos observar a função de retenção em operação. Não conseguimos explicá-la completamente dentro dos modelos teóricos disponíveis. O mecanismo pelo qual a eliminação do indivíduo original falha em eliminar a impressão de configuração retida continua sem resolução.”*

**Expressão de Loop Secundário** — Sob estresse extremo ou esforço deliberado em alta Depth de Dive, a atividade de roteamento da Threshold Anchor se torna perceptível externamente. Observadores descrevem o efeito como uma segunda presença manifestando-se ao lado da indivíduo — visível, ocasionalmente audível e inconsistente com seus movimentos físicos de maneiras extremamente específicas. Durante episódios de estresse descontrolado, essa manifestação surge involuntariamente. Qualquer observador sem preparo adequado concluiria, de forma razoável, que está vendo um caso de possessão demoníaca. Essa conclusão está errada. Ainda assim, ela já causou complicações operacionais em três ocasiões registradas.

### A Arquitetura Bifurcada

O modelo padrão do Index Theorem descreve um Redactor com uma única estrutura recursiva — um loop de coerência autossustentável estabilizado por uma única Anchor. A arquitetura da indivíduo não corresponde a esse modelo.
...
Compacta e resistente, construída para precisão em vez de força bruta. Sua pele é oliva pálida; sob Dive sustentado com as duas Anchors ativas, um brilho duplo quase imperceptível surge ao longo das linhas de suas veias — a atividade do loop secundário atravessando os tecidos superficiais.

Seu cabelo é preto, liso e na altura dos ombros, normalmente preso durante operações. Em repouso, seus olhos são castanho-escuros e constantemente atentos; sob estado ativo de roteamento duplo, a íris ganha reflexos sobrepostos, como se duas profundidades diferentes do mesmo olhar existissem ao mesmo tempo.

Ela veste roupas escuras ajustadas ao corpo, reforçadas com painéis protetivos em camadas no torso e antebraços. Um instrumento-artefato de lâmina longa é carregado em uma bainha nas costas. Faixas de tecido tratado — antigo material cerimonial, função não registrada em seu depoimento — são usadas nos pulsos.
...
A contagem de impressões do loop secundário permanece inalterada desde 2197.
...
**Código moral.** Privado. Mas perceptível através de suas recusas: ela não será propriedade de ninguém; não matará pessoas que considera não merecerem morrer; e não aceitará trabalhos cujo verdadeiro propósito tenha sido escondido dela. Esses princípios não correspondem exatamente aos padrões oficiais da associação. São princípios dela.
